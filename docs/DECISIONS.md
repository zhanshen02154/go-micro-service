...
### 最终方案
取消ElasticSearch和Jaeger，将k8s-node4的Prometheus Agent和Apisix、Redis迁移到资源更充足的服务器。
- 日志: 用阿里云SLS，维持按比例采集策略确保不超出免费额度。
- Jaeger: 改用阿里云的Opentelemetry，卸载自建Jaeger。
### 采纳理由
之前自建目的在于熟悉搭建步骤却导致成本激增，新买的k8s-node4几乎空转，prometheus还是Agent模式几乎不占用资源，k8s-node3又被ES占用大量内存（2.5G-2.7G），总使用率达57%。 

Jaeger和日志移到阿里云可降低ECS服务器负荷，而且之前已经实现按比例采集，用量可控，实现零成本可观测性。
...
### 方案A
利用现成的指数退避重试组件和包装器做重试，通过元数据传递一个“开关”决定是否重试。
#### 优点
- 与go micro框架无缝集成。
- 有开关保障，避免全局重试。
#### 缺点
- 每次执行subscriber handler会重复执行日志、链路追踪及指标，改动幅度较大。
### 方案B
- 编写一个重试接口并引入到serviceContext，在应用层做重试。
#### 优点
- 避开包装器传播错误导致日志记录不准确的陷阱，精准控制服务重试调用链。
#### 缺点
- 改动幅度较大，应用层要重新编排。
### 最终方案
方案B
### 采纳理由
- 避开WrapSubscriber所有包装器重复执行的问题，不影响不需要重试的方法。
...
### 最终方案
取消编译镜像时注入，手动创建非root用户并以最低权限运行，deployment设置Secret注入环境变量，设置安全上下文，将alpine 3.22.2升级为3.23.2。
### 采纳理由
通过Secret注入环境变量减少敏感数据暴露面，以最低权限运行防止容器拥有巨大权限。

## ADR-015: 分布式锁由ETCD改为Redis
### 日期
2026年1月16日
### 状态
已采纳
...
### 最终方案
分布式锁的底层组件替换为Redis，在新购的k8s-node4部署redis，开放6379端口给阿里云的Prometheus接入。
### 采纳理由
Redis是内存数据库，ETCD是高可用分布式锁，总体上ETCD更可靠，但刷盘操作导致CPU飙升挤占MySQL操作空间，6.0负载测试表明系统服务处理速度极快且具备完善的容错机制，所以用Redis取代ETCD作为分布式锁
组件并无大碍。

## ADR-014: 部署Prometheus
### 日期
2026年1月12日
### 状态
已采纳
### 背景
5.0版完善了Jaeger，虽然负载测试表现良好但仍存在长尾请求，需要引入监控。
### 最终方案
K8S集群仅部署Prometheus Agent，对接阿里云的Prometheus接口，减轻服务器的压力。新购一台服务器，用节点标签强制调度到这台服务器，集群内的监控仍由metrics server完成，仅保留服务的监控，为服务添加prometheus注解让其抓取。
### 采纳理由
阿里云免费额度的Prometheus完全够用，可以实现零成本监控，Agent模式下的Prometheus不用做持久化处理，系统负荷大幅降低。
...
### 最终方案
GORM、框架底层、GRPC请求、发布/订阅日志调整级别，分为info、warn、error，在配置文件更改日志级别后重启即可减少大量日志，实际上只要控制zap组件的日志级别即可，无需在各日志组件里判断和设置日志级别。
### 采纳理由
全局日志级别有助于快速排错，防止被大量info日志淹没，加上fluent bit的按比例采集LUA脚本，大幅降低日志收集系统的压力。

## ADR-012: 同步生产者改为异步生产者
### 日期
2025年12月30日
### 状态
已采纳
...
### 最终方案
- 改为异步生产者，监听两个管道（成功和失败），回调函数处理记录日志，发送失败的推入死信队列，让两个服务同时监听死信队列完成补偿操作。
### 采纳理由
- 异步生产者仅需发送到管道再由管道发送，延迟更短，能有效提升消费速度。复杂度上有所提升，事件侦听器改动较大但收益显著，加上死信队列和幂等性保障，总体上可靠。
### 风险及应对措施
- 风险1: 发布的链路追踪到Client即结束，无法得知是否发布成功。
- 应对措施: Client的发布逻辑和Wrapper用go micro官方的包装器，在回调函数中开一个Span即可追踪。
