...
### 采纳理由
全局日志级别有助于快速排错，防止被一大堆debug和info淹没，降低fluent bit收集日志的压力。
...
已采纳
### 背景
通过jaeger和发布日志分析得知处理的延迟集中在发送到kafka（107--200ms），加上WaitforAll导致整体延迟增加，最明显的是扣减库过程消费速度不到200个/s，业务逻辑处理不到15ms，所以要改成异步生产者。
### 最终方案
